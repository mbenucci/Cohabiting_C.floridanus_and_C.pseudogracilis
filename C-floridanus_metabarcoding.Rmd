...
ggplot(melt.occup.edna, aes(sample_id, value)) +
  geom_bar(stat="identity", position="stack", aes(fill=order)) +
  facet_grid(.~time)
dev.off()
```
...
rich.kick = as.data.frame(occupancy.kick[,!c(colnames(occupancy.kick) %in% c("sample_type","site"))] %>% group_by(time, sample_id) %>% summarise_all(list(sum)))
rownames(rich.kick) = paste(rich.kick$sample_id, rich.kick$time, sep="_")

rich.kick = rowSums(rich.kick[,!c(colnames(rich.kick) %in% c("sample_id","time"))])
