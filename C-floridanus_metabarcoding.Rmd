...
```{r}
#occup.kick.info = occup.kick[,c(colnames(occup.kick) %in% c("time","site","sample_id","sample_type","predator_id"))]
#occup.kick = occup.kick[,!c(colnames(occup.kick) %in% c("time","site","sample_id","sample_type","predator_id"))]/6

#occup.kick = cbind(occup.kick.info, occup.kick)
```
...
png("R_images/NMDS_guts_publication.png", width = 4000, height = 4000, units = "px", res = 310)
...
points(meta.guts, select=which(fact_predator.guts[,1]=="C.pseudogracilis" & fact_time.guts[,1]=="May"),col="black", pch=17)
points(meta.guts, select=which(fact_predator.guts[,1]=="C.pseudogracilis" & fact_time.guts[,1]=="Oct"),col="black", pch=24)
points(meta.guts, select=which(fact_predator.guts[,1]=="C.floridanus" & fact_time.guts[,1]=="May"),col="black", pch=15)
points(meta.guts, select=which(fact_predator.guts[,1]=="C.floridanus" & fact_time.guts[,1]=="Oct"),col="black", pch=22)
ordispider(meta.guts, group=fact_predator.guts[,1],col=c("red","orange"),label=T)
ordiellipse(meta.guts, group=fact_predator.guts[,1],kind="ehull",lty=c(1,5),label=F,lwd=1)
text(1,-1,label=paste0("Stress: ",round(meta.guts$stress, 6)))
...
# Common legend
plot.new()
legend("top", legend=c("Crangonyx pseudogracilis - May","Crangonyx pseudogracilis - Oct","Crangonyx floridanus - May","Crangonyx floridanus - Oct"), col=c("black","black","red","red"), pch=c(17,24,15,22), cex=2, ncol=2)
dev.off()
```
...
png("R_images/NMDS_community_pubblication.png", width = 6000, height = 6000, units = "px", res = 310)
lay.matrix = matrix(c(1,2), ncol=2, nrow=1, byrow=T)
layout(lay.matrix, heights=c(2), widths=c(2,2))

plot(meta.kick, type="n", xlim=c(-0.6,0.6), ylim=c(-0.6,0.6))
points(meta.kick, select=which(fact_time.kick[,1]=="May"),col="black",pch=20)
points(meta.kick, select=which(fact_time.kick[,1]=="Oct"),col="black",pch=1)
ordispider(meta.kick, group=fact_time.kick[,1],col=c("green","darkgreen"),label=T)
ordiellipse(meta.kick, group=fact_time.kick[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(-0.5,-0.4, label=paste0("Stress: ", round(meta.kick$stress,6)))

plot(meta.kick.edna, type="n")#, xlim=c(-1.5,1.5), ylim=c(-1,1.1))
points(meta.kick.edna, select=which(fact_time.edki[,1]=="May"),col="black",pch=20)
points(meta.kick.edna, select=which(fact_time.edki[,1]=="Oct"),col="black",pch=1)
orditorp(meta.kick.edna, display="species")
ordispider(meta.kick.edna, group=fact_type.edki[,1],col=c("blue","green"),label=T)
ordiellipse(meta.kick.edna, group=fact_type.edki[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(0, -1, label=paste0("Stress: ",round(meta.kick.edna$stress, 6)))
dev.off()
```
...
barcode_alignment=readDNAMultipleAlignment("C.floridanus_confirmation/Crangonyx_barcoding/c.flor-c.pse_full_SANGER.fasta")
