...
rich.kick = as.data.frame(occupancy.kick[,!c(colnames(occupancy.kick) %in% c("sample_type","sample_id"))] %>% group_by(time, site) %>% summarise_all(list(sum)))
...
adonis.kick = as.data.frame(occupancy.kick[,!c(colnames(occupancy.kick) %in% c("sample_id","sample_type"))] %>% group_by(site, time) %>% summarise_all(list(sum)))
...
# Common legend
plot.new()
legend("top", legend=c("May","Oct"), col=c("black","black","blue","blue"), pch=c(19,1), cex=2, ncol=1)
dev.off()
```
