...
melt.kick[melt.kick$order == "Amphipoda",] %>% 
  ...
  theme_bw() +
  facet_grid(time~., scales="free_x") +lay.matrix = matrix(c(1,2,3,3), ncol=2, nrow=2, byrow=T)
...
kick.summary = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("site","REPLICATE","location"))] %>% group_by(sample_id, time,sample_type) %>% summarise_all(list(sum)))
