...
# second level to test for May =0.00068
thres.may =  0.00031

# second level to test for October = 0.00073
thres.oct =  0.0039
```

Considering these values of contamination, decided to use the threshold of 0.03% for May, and of 0.39% for October

```{r creating pre and post filter}
may.clean = subset(may.clean[,-c(grep("POS|NEG", colnames(may.clean))),])
may.pre.filter = as.data.frame(t(may.clean))
may.pre.filter = may.pre.filter[,]/rowSums(may.pre.filter)

oct.clean = subset(oct.clean[,-c(grep("POS|NEG", colnames(oct.clean))),])
