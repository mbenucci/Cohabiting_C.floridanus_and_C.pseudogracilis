...
pack.list = c("adegenet","aod","ape","bipartite","boot","cowplot","devtools","dplyr","EcoSimR","ggplot2","ggsignif","gplots","grid","gridExtra","iNEXT","lattice","lme4","lmtest","MASS","mgcv","msa","network","pegas","plyr","reshape2","spaa","stringi","tidyr","UpSetR","vegan","zoo")
...
```{r extracting Chasewater samples}
...
Calculating the read depth before any QC applied.

```{r mean read depth, include=F}
may17.reads = subset(may17.reads, select=colSums(may17.reads) > 0)
oct17.reads = subset(oct17.reads, select=colSums(oct17.reads) > 0)

## Mean values + standard error May
mean(may.depth$value)
sd(may.depth$value)/sqrt(length(may.depth$value))

## Mean values + SE May eDNA
mean(unlist(may.depth[grep("e",may.depth$variable),"value"]))
sd(unlist(may.depth[grep("e",may.depth$variable),"value"]))/sqrt(length(unlist(may.depth[grep("e",may.depth$variable),"value"])))

## Mean values + SE May gut contents
mean(unlist(may.depth[grep("^CH.",may.depth$variable),"value"]))
sd(unlist(may.depth[grep("^CH.",may.depth$variable),"value"]))/sqrt(length(unlist(may.depth[grep("^CH.",may.depth$variable),"value"])))

## Mean values + standard error October
mean(oct.depth$value)
sd(oct.depth$value)/sqrt(length(oct.depth$value))

## Mean values + SE Oct eDNA
mean(unlist(oct.depth[grep("e",oct.depth$variable),"value"]))
sd(unlist(oct.depth[grep("e",oct.depth$variable),"value"]))/sqrt(length(unlist(oct.depth[grep("e",oct.depth$variable),"value"])))

## Mean values + SE Oct gut contents
mean(unlist(oct.depth[grep("^CH.",oct.depth$variable),"value"]))
sd(unlist(oct.depth[grep("^CH.",oct.depth$variable),"value"]))/sqrt(length(unlist(oct.depth[grep("^CH.",oct.depth$variable),"value"])))
...
# 2. Setting eDNA and gut contents data

Before continuing with data analysis we add the metadata for sample type, sample id and time. We then split the data sets into gut contentns and eDNA.
...
Initial predator assignment was done using taxa with a proportion greater or equal to 90% of each sample. We now check those samples that had a lower values. In May data set we managed to set all predator ids, while in October we need to sort out 22 predator ids. These 22 are going to bechecked using a proportion greater or equal to 80%, and the one that will still be missing following that will be manually added.
...
Calculating predator:prey DNA ratio across all gut contents samples

```{r predator:prey ratio set up}
#sd(x)/sqrt(length(x)
pred.cf.may = may.post.filter[may.post.filter$predator_id == "Crangonyx_floridanus","Crangonyx_floridanus"]
prey.cf.may = rowSums(may.post.filter[may.post.filter$predator_id == "Crangonyx_floridanus", !c(colnames(may.post.filter) %in% c("Crangonyx_floridanus","time","site","sample_id","sample_type","predator_id"))])

pred.cp.may = may.post.filter[may.post.filter$predator_id == "Crangonyx_pseudogracilis","Crangonyx_pseudogracilis"]
prey.cp.may = rowSums(may.post.filter[may.post.filter$predator_id == "Crangonyx_pseudogracilis", !c(colnames(may.post.filter) %in% c("Crangonyx_pseudogracilis","time","site","sample_id","sample_type","predator_id"))])

pred.cf.oct = oct.post.filter[oct.post.filter$predator_id == "Crangonyx_floridanus","Crangonyx_floridanus"]
prey.cf.oct = rowSums(oct.post.filter[oct.post.filter$predator_id == "Crangonyx_floridanus", !c(colnames(oct.post.filter) %in% c("Crangonyx_floridanus","time","site","sample_id","sample_type","predator_id"))])

pred.cp.oct = oct.post.filter[oct.post.filter$predator_id == "Crangonyx_pseudogracilis","Crangonyx_pseudogracilis"]
prey.cp.oct = rowSums(oct.post.filter[oct.post.filter$predator_id == "Crangonyx_pseudogracilis", !c(colnames(oct.post.filter) %in% c("Crangonyx_pseudogracilis","time","site","sample_id","sample_type","predator_id"))])
```


```{r predator:prey ratio}
print("Mean C.floridanus predator - May")
mean(pred.cf.may)
sd(pred.cf.may)/sqrt(length(pred.cf.may))

print("Mean C.floridanus preys - May")
mean(prey.cf.may)
sd(prey.cf.may)/sqrt(length(prey.cf.may))

print("Mean C.pseudogracilis predator - May")
mean(pred.cp.may)
sd(pred.cp.may)/sqrt(length(pred.cp.may))

print("Mean C.pseudogracilis preys - May")
mean(prey.cp.may)
sd(pred.cp.may)/sqrt(length(pred.cp.may))

print("Mean C.floridanus predator - Oct")
mean(pred.cf.oct)
sd(pred.cf.oct)/sqrt(length(pred.cf.oct))

print("Mean C.floridanus prey - Oct")
mean(prey.cf.oct)
sd(prey.cf.oct)/sqrt(length(prey.cf.oct))

print("Mean C.pseudogracilis predator - Oct")
mean(pred.cp.oct)
sd(pred.cp.oct)/sqrt(length(pred.cp.oct))

print("Mean C.pseudogracilis prey - Oct")
mean(prey.cp.oct)
sd(prey.cp.oct)/sqrt(length(prey.cp.oct))
```
...
We divide gut contents and eDNA samples, we then save them into separate files.
...
We split the datasets into two separate frame that we are going to use downstream.
...
Done all the QC and filtration we now start analysis the gut contents interactions. First the prey composition in the gut contents need to be converted into ratios.
...
ratio.guts[ratio.guts$time == "May", "sample_id"]
ratio.guts[ratio.guts$time == "Oct", "sample_id"]
...
```{r transforming into presence/absence}
# removing `sample_type` and `sample_id` variables and transforming detections into presence/absence
...
abund.guts.site[grep("Crangonyx_floridanus", abund.guts.site$predator_id),"predator_id"] <- "Cf"
abund.guts.site[grep("Crangonyx_pseudogracilis", abund.guts.site$predator_id),"predator_id"] <- "Cp"

abund.guts[grep("Crangonyx_floridanus", abund.guts$predator_id),"predator_id"] <- "Cf"
abund.guts[grep("Crangonyx_pseudogracilis", abund.guts$predator_id),"predator_id"] <- "Cp"
...
for (i in seq(1:nrow(abund.guts))){
  if (abund.guts[i,"Crangonyx"] >= 1){
    if (abund.guts[i,"predator_id"] == "Cp"){
      print(abund.guts[i,"predator_id"])
      abund.guts[i,"Crangonyx_floridanus"] <- (abund.guts[i,"Crangonyx"] + abund.guts[i,"Crangonyx_floridanus"])
    } else if (abund.guts[i,"predator_id"] == "Cf"){
      print(abund.guts[i,"predator_id"])
      abund.guts[i,"Crangonyx_pseudogracilis"] <- (abund.guts[i,"Crangonyx"] + abund.guts[i,"Crangonyx_pseudogracilis"])
    }
  }
}

abund.guts = abund.guts[,!c(colnames(abund.guts) %in% "Crangonyx")]
```
...
png("R_images/guts_predator_NMDS_euclidean.png", width = 4000, height = 4000, units = "px", res = 300)
plot(meta.guts, type="n", xlim=c(-1,1.5), ylim=c(-1.5,2))
points(meta.guts, select=which(fact_predator.guts[,1]=="Cp" & fact_time.guts[,1]=="May"),col="black", pch=17)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cp" & fact_time.guts[,1]=="Oct"),col="black", pch=24)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cf" & fact_time.guts[,1]=="May"),col="blue", pch=15)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cf" & fact_time.guts[,1]=="Oct"),col="blue", pch=22)
ordispider(meta.guts, group=fact_predator.guts[,1],col=c("blue","orange"),label=T)
ordiellipse(meta.guts, group=fact_time.guts[,1],kind="ehull",lty=c(1,5),label=F,lwd=1)
...
We now analyse the niche breadth of _C. pseudogracilis_ and _C. floridanus_ across the lake and the two seasons. Furthermore we calculate the niche overlap between the two amphipods.

```{r niche breadth of amphipods species}
...
breadth.cfcp[grep("Crangonyx", rownames(breadth.cfcp)),] <- 0

breadth.cfcp[,1] = breadth.cfcp[,1]/107
...
```{r niche breadth of amphipods species with IGP}
# summing prey DNA reads by predator ID
breadth.cfcp.2 = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_type","sample_id","site","time"))] %>% group_by(predator_id) %>% summarise_all(list(sum)))

rownames(breadth.cfcp.2) = breadth.cfcp.2$predator_id

breadth.cfcp.2 = t(breadth.cfcp.2[,!c(colnames(breadth.cfcp.2) %in% c("predator_id"))])

breadth.cfcp.2[,1] = breadth.cfcp.2[,1]/107
breadth.cfcp.2[,2] = breadth.cfcp.2[,2]/107

niche.width(breadth.cfcp.2, method="levins")
niche.width(breadth.cfcp.2, method="shannon")

(niche.width(breadth.cfcp.2, method="levins")[1]-1)/(nrow(breadth.cfcp.2)-1)
(niche.width(breadth.cfcp.2, method="levins")[2]-1)/(nrow(breadth.cfcp.2)-1)

niche.overlap(breadth.cfcp.2, method="pianka")
```
...
```{r plot order}
col.network=c("red","red","red","khaki3","khaki3","cyan3","yellow","tan4","tan4","darkorchid3","darkgreen","cyan3","tan4","cyan3")
```
...
plotweb(food.web, y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lablength=5, low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.07, text.rot=90, col.high="black", col.low=col.network, high.y=2, low.y=1)
#legend("bottom", legend=name.legend, fill=col.legend, bty="n", border="black",ncol=3, xpd=T)
dev.off()
...
```{r community composition eDNA}
unique(melt.occup.edna$variable)

melt.occup.edna$order = "NA"
melt.occup.edna[grep("Hydracarina|Limnesia|Platynothrus", melt.occup.edna$variable),"order"] = "Arachnida"
melt.occup.edna[grep("Chydorus|Daphnia|Diaphanosoma|Eurycercus|Polyphemus|Sida", melt.occup.edna$variable),"order"] = "Cladocera"
melt.occup.edna[grep("Cyclops|Eucyclops|Eudiaptomus|Maxillopoda|Mesocyclops", melt.occup.edna$variable), "order"] = "Cyclopoida"
melt.occup.edna[grep("Agabus|Dryopidae|Haliplus|Hyphydrus|Limnius|Platambus", melt.occup.edna$variable), "order"] = "Coleoptera"
melt.occup.edna[grep("Asellus", melt.occup.edna$variable), "order"] = "Isopoda"
melt.occup.edna[grep("Corophium|Crangonyx", melt.occup.edna$variable), "order"] = "Amphipoda"
melt.occup.edna[grep("Ceratopogonidae|Chironomidae|Cricotopus|Endochironomus|Limoniidae|Polypedilum|Stratiomys|Tabanidae|Tipulidae", melt.occup.edna$variable), "order"]= "Diptera"
melt.occup.edna[grep("Caenis|Procleon", melt.occup.edna$variable), "order"] = "Ephemeroptera"
melt.occup.edna[grep("Corixidae", melt.occup.edna$variable), "order"] = "Hemiptera"
melt.occup.edna[grep("Bereobdella|Erpobdella|Glossiphonia|Haemopis|Helobdella", melt.occup.edna$variable), "order"] = "Hirudinea"
melt.occup.edna[grep("Hydra", melt.occup.edna$variable), "order"] = "Hydrozoa"
melt.occup.edna[grep("Bithynia|Dreissena|Lymnaea|Pisidium|Planorbis|Potamopyrgus|Segmentina|Sphaerium|Valvata", melt.occup.edna$variable), "order"] = "Mollusca"
melt.occup.edna[grep("Limnodrilus|Lumbriculus|Tubifex", melt.occup.edna$variable), "order"] = "Oligochaeta"
melt.occup.edna[grep("Cypridopsis|Eucypris", melt.occup.edna$variable), "order"] = "Ostracoda"
melt.occup.edna[grep("Ascomorpha|Euchlanis|Kellicottia|Keratella|Ploima|Polyarthra|Synchaeta", melt.occup.edna$variable), "order"] = "Rotifera"
melt.occup.edna[grep("Ceraclea|Hydropsychidae|Psychomiidae|Psychomyia", melt.occup.edna$variable), "order"] = "Trichoptera"
melt.occup.edna[grep("Coenagrionidae", melt.occup.edna$variable), "order"] = "Zygoptera"

cols_edna = c("darkorange","greenyellow","yellow","darkgreen","red","purple","green3","khaki4")

png("R_images/eDNA_community.png", width = 3000, height = 3000, units = "px", res=300)
ggplot(melt.occup.edna, aes(site, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=order)) +
  scale_x_discrete(limits=c("eCH1","eCH2","eCH3","eCH4","eCH5")) +
  coord_flip() +
  labs(y="Community composition (%)", x="") +
  facet_grid(time~.)
dev.off()
```
...
NMDS and PERMANOVA of eDNA samples

```{r adonis eDNA}
adonis.edna = as.data.frame(occup.edna[,!c(colnames(occup.edna) %in% c("sample_id","sample_type","predator_id"))] %>% group_by(site, time) %>% summarise_all(list(sum)))

fact_time.edna = matrix(c(adonis.edna$time), ncol=1)
fact_site.edna = matrix(c(adonis.edna$site), ncol=1)

rownames(adonis.edna) = paste(adonis.edna$site, adonis.edna$time, sep="_")

adonis.edna = vegdist(as.matrix(adonis.edna[,!c(colnames(adonis.edna) %in% c("site","time"))]))

adonis(adonis.edna~fact_time.edna, permutations = 9999, method="jaccard")
```

```{r PERMANOVA of eDNA}
meta.edna = metaMDS(adonis.edna, trymax=200, distance="jaccard")
```

```{r NMDS plot of kick-samples}
#full symbols for May, hollow symbols for October
#png("R_images/eDNA_NMDS_jaccard.png", width = 3000, height = 3000, units = "px", res = 300)
plot(meta.edna, type="n", xlim=c(-0.6,0.6), ylim=c(-0.6,0.6))
points(meta.edna, select=which(fact_time.edna[,1]=="May"),col="black",pch=20)
points(meta.edna, select=which(fact_time.edna[,1]=="Oct"),col="black",pch=1)
...
text(-0.5,-0.4, label=paste0("Stress: ", round(meta.edna$stress,6)))
#dev.off()
```

# 7. Diversity analysis in kick-samples
...
```{r sorting variables}
ch.kick$sample_id = ch.kick$tube_id
ch.kick$site = paste0(ch.kick$location, ch.kick$site)
```

```{r joining frames}
abund.kick = ch.kick[,!c(colnames(ch.kick) %in% "tube_id")]

# Sorting out genus names to be consistent with eDNA and gut contents data
colnames(abund.kick)
colnames(abund.kick) = gsub("_sp.", "", colnames(abund.kick))
```

NMDS of kick samples.
...
raref.kick = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("sample_id"))] %>% group_by(site, sample_type, time, location) %>% summarise_all(list(sum)))

# setting rownames and removing extra variables
rownames(raref.kick) = paste(raref.kick$site, raref.kick$time, sep="_")
raref.kick = raref.kick[,!c(colnames(raref.kick) %in% c("site","sample_type","time","location"))]
...
occup.kick = abund.kick[,!c(colnames(abund.kick) %in% c("location"))]

occup.kick[,!c(colnames(occup.kick) %in% c("time","sample_id","site","sample_type"))][occup.kick[,!c(colnames(occup.kick) %in% c("time","sample_id","site","sample_type"))] > 0] <- 1

rich.kick = as.data.frame(occup.kick[,!c(colnames(occup.kick) %in% c("sample_type","sample_id"))] %>% group_by(time, site) %>% summarise_all(list(sum)))
rownames(rich.kick) = paste(rich.kick$site, rich.kick$time, sep="_")

rich.kick = rowSums(rich.kick[,!c(colnames(rich.kick) %in% c("site","time"))])
...
adonis.kick = as.data.frame(occup.kick[,!c(colnames(occup.kick) %in% c("sample_id","sample_type"))] %>% group_by(site, time) %>% summarise_all(list(sum)))

fact_time.kick = matrix(c(adonis.kick$time), ncol=1)
fact_site.kick = matrix(c(adonis.kick$site), ncol=1)

rownames(adonis.kick) = paste(adonis.kick$site, adonis.kick$time, sep="_")

adonis.kick = vegdist(as.matrix(adonis.kick[,!c(colnames(adonis.kick) %in% c("site","time"))]))

adonis(adonis.kick~fact_time.kick, permutations = 9999, method="jaccard")
```
...
occup.kick.edna = full_join(occup.edna[,!c(colnames(occup.edna) %in% "predator_id")], occup.kick)
occup.kick.edna[is.na(occup.kick.edna)] <- 0
#View(occup.kick.edna)
occup.kick.edna = as.data.frame(occup.kick.edna[,!c(colnames(occup.kick.edna) %in% c("sample_id"))] %>% group_by(sample_type, site, time) %>% summarise_all(list(sum)))

adonis.kick.edna = occup.kick.edna[,!c(colnames(occup.kick.edna) %in% c("site","sample_type","time"))]

fact_time.edki = matrix(c(occup.kick.edna$time), ncol=1)
fact_type.edki = matrix(c(occup.kick.edna$sample_type), ncol=1)

adonis(adonis.kick.edna~fact_time.edki*fact_type.edki, permutations = 9999, method = "jaccard")
```
...
adonis.guts.edna.kick = occup.gut.edna.kick[,!c(colnames(occup.gut.edna.kick) %in% c("site","sample_type","time","predator_id"))]
...
adonis(adonis.guts.edna.kick~fact_type.full*fact_time.full, permutations = 9999, method = "jaccard")
```
...
List of colours and species for summary barplot:

"blue" = Amphidoda,
"darkorange" = Cladocera, 
"cyan3" = Coleoptera,
"greenyellow" = Cyclopoida,
"yellow" = Diptera,
"deeppink" = Ephemeroptera,
"plum2" = Hemiptera,
"grey50" = Hirudinea,
"darkgreen" = Hydrozoa,
"red" = Isopoda,
"purple" = Mollusca,
"aquamarine" = Oligochaeta,
"green3" = Ostracoda,
"khaki4" = Rotifera,
"brown" = Trichoptera,
"gainsboro" = Tricladida,
"black" = Zygoptera,

List of colours and species for the kick-sample barplot
"olivedrab" = Corophium,
"dodgerblue" = Crangonyx sp.,
"darkblue" = Crangonyx floridanus,
"cyan" = Crangonyx pseudogracilis
...
occup.melt$order = "NA"
occup.melt[grep("Corophium", occup.melt$variable), "order"] = "Corophiida"
occup.melt[grep("Crangonyx", occup.melt$variable), "order"] = "Crangonyctidae"
...
occup.melt[grep("Schmidtea", occup.melt$variable), "order"] = "Tricladida"
occup.melt[grep("Coenagrionidae", occup.melt$variable), "order"] = "Zygoptera"

occup.melt[occup.melt$order == "NA","variable"]

cols_barplot = c("blue","olivedrab","darkorange","cyan3","greenyellow","yellow","deeppink","plum2","grey50","darkgreen","red","purple","aquamarine","green3","khaki4","brown","gainsboro","black")

png("R_images/barplot_summary_by_order.png", width = 3000, height = 3000, units = "px", res = 300)
occup.melt %>% mutate(order=factor(order, levels=c("Crangonyctidae", "Corophiida", "Cladocera", "Coleoptera", "Cyclopoida", "Diptera", "Ephemeroptera", "Hemiptera", "Hirudinea", "Hydrozoa", "Isopoda", "Mollusca", "Oligochaeta", "Ostracoda", "Rotifera", "Trichoptera", "Tricladida", "Zygoptera"))) %>% ggplot(aes(site, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=order)) +
  scale_fill_manual(values=cols_barplot, name = "") +
  scale_x_discrete(limits=c("CH6","CH5","CH4","CH3","CH2","CH1")) +
...
```{r barplot of gut contents}
guts.melt=melt(occup.gut.edna.kick[occup.gut.edna.kick$sample_type=="guts",])

unique(guts.melt$variable)

# We split Amphipoda in Crangonyx and Corophium
guts.melt[grep("Corophium", guts.melt$variable), "order"] = "Corophiida"
guts.melt[grep("Crangonyx", guts.melt$variable), "order"] = "Crangonyctidae"
guts.melt[grep("Chydorus|Daphnia|Diaphanosoma|Eurycercus|Polyphemus|Sida", guts.melt$variable), "order"] = "Cladocera"
guts.melt[grep("Hydracarina|Limnesia|Platynothrus", guts.melt$variable),"order"] = "Arachnida"
guts.melt[grep("Cyclops|Eucyclops|Eudiaptomus|Maxillopoda|Mesocyclops", guts.melt$variable), "order"] = "Cyclopoida"
guts.melt[grep("Agabus|Dryopidae|Haliplus|Hyphydrus|Limnius|Platambus", guts.melt$variable), "order"] = "Coleoptera"
guts.melt[grep("Ceratopogonidae|Chironomidae|Cricotopus|Endochironomus|Limoniidae|Polypedilum|Stratiomys|Tabanidae|Tipulidae", guts.melt$variable), "order"]= "Diptera"
guts.melt[grep("Caenis|Procleon", guts.melt$variable), "order"] = "Ephemeroptera"
guts.melt[grep("Corixidae", guts.melt$variable), "order"] = "Hemiptera"
guts.melt[grep("Bereobdella|Erpobdella|Glossiphonia|Haemopis|Helobdella", guts.melt$variable), "order"] = "Hirudinea"
guts.melt[grep("Hydra", guts.melt$variable), "order"] = "Hydrozoa"
guts.melt[grep("Asellus", guts.melt$variable), "order"] = "Isopoda"
guts.melt[grep("Bithynia|Dreissena|Lymnaea|Pisidium|Planorbis|Potamopyrgus|Segmentina|Sphaerium|Valvata", guts.melt$variable), "order"] = "Mollusca"
guts.melt[grep("Limnodrilus|Lumbriculus|Tubifex", guts.melt$variable), "order"] = "Oligochaeta"
guts.melt[grep("Cypridopsis|Eucypris", guts.melt$variable), "order"] = "Ostracoda"
guts.melt[grep("Ascomorpha|Euchlanis|Kellicottia|Keratella|Ploima|Polyarthra|Synchaeta", guts.melt$variable), "order"] = "Rotifera"
guts.melt[grep("Ceraclea|Hydropsychidae|Psychomiidae|Psychomyia", guts.melt$variable), "order"] = "Trichoptera"
guts.melt[grep("Schmidtea", guts.melt$variable), "order"] = "Tricladida"
guts.melt[grep("Coenagrionidae", guts.melt$variable), "order"] = "Zygoptera"

unique(guts.melt$order)

cols_gut = c("blue","olivedrab","darkorange","greenyellow","yellow","purple","aquamarine","gainsboro")

png("R_images/barplot_summary_guts_by_order.png", width = 3000, height = 3000, units = "px", res = 300)
guts.melt[guts.melt$value > 0,] %>% mutate(order=factor(order, levels=c("Crangonyctidae", "Corophiida", "Cladocera", "Cyclopoida", "Diptera", "Mollusca", "Oligochaeta", "Tricladida"))) %>%  ggplot(aes(predator_id, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=order)) +
  scale_fill_manual(values=cols_barplot, name = "") +
  theme_bw() +
  coord_flip() +
...
cols_amphipod = c("olivedrab","dodgerblue","darkblue","cyan")

png("R_images/barplot_summary_amphipod.png", width = 3000, height = 3000, units = "px", res = 300)
melt.kick[melt.kick$order == "Amphipoda",] %>% 
  mutate(variable = factor(variable, levels=c("Corophium","Crangonyx","Crangonyx_floridanus","Crangonyx_pseudogracilis"))) %>% 
  ggplot(aes(site, value, group=time)) +
  geom_bar(stat="identity", position="stack", aes(fill=variable)) +
  scale_fill_manual(values = cols_amphipod, name = "", labels=c("Corophium","Crangonyx sp.","C.floridanus","C.pseudogracilis")) +
  theme_bw() +
...
guts.summary = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_id"))] %>% group_by(site, time, predator_id, sample_type) %>% summarise_all(list(sum)))
edna.summary = as.data.frame(occup.edna[,!c(colnames(occup.edna) %in% c("sample_id","predator_id"))] %>% group_by(site, time, sample_type) %>% summarise_all(list(sum)))
kick.summary = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("REPLICATE","location","sample_id"))] %>% group_by(site, time, sample_type) %>% summarise_all(list(sum)))
...
#Saving final output
write.csv(final.data, "R_outputs/Crangonyx_summary_table.csv")
```

# 11. Publication plots

```{r guts NMDS publication}
...
plot(meta.guts, type="n", xlim=c(-1,1.5), ylim=c(-1,2), cex.axis=2, cex.lab=1.5)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cp" & fact_time.guts[,1]=="May"),col="black",pch=17,cex=2)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cp" & fact_time.guts[,1]=="Oct"),col="black",pch=24,cex=2)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cf" & fact_time.guts[,1]=="May"),col="black",pch=15,cex=2)
points(meta.guts, select=which(fact_predator.guts[,1]=="Cf" & fact_time.guts[,1]=="Oct"),col="black",pch=22,cex=2)
ordispider(meta.guts,group=fact_predator.guts[,1],col=c("red","orange"),label=T,cex=2)
ordiellipse(meta.guts,group=fact_time.guts[,1],kind="ehull",lty=c(1,5),label=F,lwd=1)
text(1.5,-1,label=paste0("Stress: ",round(meta.guts$stress,4)),cex=1.5)
text(0.5,1.3,label="May",cex=2.5)
text(-0.9,-0.3,label="Oct",cex=2.5)
text(-1.8,1.9,label="a)",cex=3)
text(0.2,1.9,label="gut contents",cex=2.5)

plot(meta.full, type="n", xlim=c(-0.6,0.6), ylim=c(-0.5,0.5), cex.axis=2, cex.lab=1.5)
points(meta.full, select=which(fact_time.full[,1]=="May"),col="black",pch=16,cex=2)
points(meta.full, select=which(fact_time.full[,1]=="Oct"),col="black",pch=1,cex=2)
ordispider(meta.full, group=fact_type.full[,1],col=c("blue","orange","green"),label=F,cex=2)
ordiellipse(meta.full, group=fact_type.full[,1],kind="ehull",lty=c(1,5,3),label=F,lwd=1)
text(0.4,-0.5,label=paste0("Stress: ",round(meta.full$stress,4)),cex=1.5)
text(0,-0.35,label="kick",cex=2.5)
text(0.45,0.35,label="eDNA",cex=2.5)
text(-0.4,0.35,label="gut contents",cex=2.5)
text(-0.67,0.45,label="b)",cex=3)

plot(meta.kick, type="n", xlim=c(-0.5,0.5), ylim=c(-0.4,0.4), cex.axis=2, cex.lab=1.5)
points(meta.kick, select=which(fact_time.kick[,1]=="May"),col="black",pch=16,cex=2)
points(meta.kick, select=which(fact_time.kick[,1]=="Oct"),col="black",pch=1,cex=2)
ordispider(meta.kick, group=fact_time.kick[,1],col=c("green","darkgreen"),label=F,cex=2)
ordiellipse(meta.kick, group=fact_time.kick[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(0.35,-0.4, label=paste0("Stress: ", round(meta.kick$stress,4)),cex=1.5)
text(-0.3,0.2, label="May",cex=2.5)
text(0.4,0.05, label="Oct",cex=2.5)
text(-0.55,0.37, label="c)", cex=3)
text(0,0.37, label="kick-samples", cex=2.5)

plot(meta.kick.edna, type="n", xlim=c(-2.7,2), ylim=c(-1,1.5), cex.axis=2, cex.lab=1.5)
points(meta.kick.edna, select=which(fact_time.edki[,1]=="May"),col="black",pch=16,cex=2)
points(meta.kick.edna, select=which(fact_time.edki[,1]=="Oct"),col="black",pch=1,cex=2)
orditorp(meta.kick.edna, display="species",type="n",cex=1.5)
ordispider(meta.kick.edna, group=fact_type.edki[,1],col=c("blue","green"),label=F,cex=2)
ordiellipse(meta.kick.edna, group=fact_type.edki[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(1,-1.4,label=paste0("Stress: ",round(meta.kick.edna$stress,4)),cex=1.5)
text(-2,1.7,label="eDNA",cex=2.5)
text(1.3,1,label="kick",cex=2.5)
text(-2.6,1.8, label="d)", cex=3)

# Common legend
plot.new()
legend("top", legend=c("C. pseudogracilis - May","C. pseudogracilis - Oct","C. floridanus - May","C. floridanus - Oct","May","Oct"), col=c("black","black","black","black"), pch=c(17,24,15,22,16,1),cex=2,ncol=3, bty="n")
dev.off()
```


```{r bipartite publication}
name.multi=c("Amphipoda","Cladocera","Copepoda","Diptera","Gastropoda","Oligochaeta","Tricladida")
col.multi=c("red","cyan3","khaki3","yellow","darkorchid3","tan4","darkgreen")

colnames(food.web) = gsub("Cf","C.floridanus", colnames(food.web))
colnames(food.web) = gsub("Cp","C.pseudogracilis", colnames(food.web))
colnames(food.web) = gsub("_", " ", colnames(food.web))

png("R_images/bipartite_network_publication.png", width=4000, height=2500, units="px", res=300)
plotweb(food.web, y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lab.dis=0.05, high.lab.dis=0.1, low.spacing=0.015, high.spacing = 0.07, text.rot=90, col.high="black", col.low=col.network, high.y=2.1, low.y=0.9)
legend("bottom", legend=name.multi, fill=col.multi, bty="n", border="black",ncol=4, xpd=T)
dev.off()
```


```{r community NMDS publication}
png("R_images/NMDS_community_pubblication.png", width = 4000, height = 4000, units = "px", res = 300)
lay.matrix = matrix(c(1,2,0,0), ncol=2, nrow=2, byrow=T)
...
ordispider(meta.kick, group=fact_time.kick[,1],col=c("green","darkgreen"),label=F,cex=2)
ordiellipse(meta.kick, group=fact_time.kick[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(0.35,-0.6, label=paste0("Stress: ", round(meta.kick$stress,4)),cex=1.5)
text(-0.3,0.2, label="May",cex=2.5)
text(0.4,0.05, label="Oct",cex=2.5)
text(-0.47,0.6, label="a)", cex=3)
text(0,0.6, label="kick-samples", cex=2.5)
...
ordispider(meta.kick.edna, group=fact_type.edki[,1],col=c("blue","green"),label=F,cex=2)
ordiellipse(meta.kick.edna, group=fact_type.edki[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(1,-2.2,label=paste0("Stress: ",round(meta.kick.edna$stress,4)),cex=1.5)
text(-2,1.7,label="eDNA",cex=2.5)
text(1.3,1,label="kick",cex=2.5)
text(-2.4,3, label="b)", cex=3)
dev.off()
```

```{r community barplot publication}
tot = occup.melt %>% mutate(order=factor(order, levels=c("Crangonyctidae", "Corophiida", "Cladocera", "Coleoptera", "Cyclopoida", "Diptera", "Ephemeroptera", "Hemiptera", "Hirudinea", "Hydrozoa", "Isopoda", "Mollusca", "Oligochaeta", "Ostracoda", "Rotifera", "Trichoptera", "Tricladida", "Zygoptera"))) %>% ggplot(aes(site, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=order)) +
  scale_fill_manual(values=cols_barplot, name = "") +
  scale_x_discrete(limits=c("CH6","CH5","CH4","CH3","CH2","CH1")) +
  scale_y_continuous(expand=c(0,0)) +
  ...
  theme(legend.position = "right", axis.text = element_text(size = 15), 
        ...
        legend.text = element_text(size=15), plot.margin=unit(c(0.25,0,0.25,0.7), unit="cm"))

kick = melt.kick[melt.kick$order == "Amphipoda",] %>% 
  mutate(variable = factor(variable, levels=c("Corophium","Crangonyx","Crangonyx_floridanus","Crangonyx_pseudogracilis"))) %>% 
  ggplot(aes(site, value, group=time)) +
  geom_bar(stat="identity", position="stack", aes(fill=variable)) +
  scale_fill_manual(values = cols_amphipod, name = "", labels=c("Corophium","Crangonyx sp.","C.floridanus","C.pseudogracilis")) +
  scale_x_discrete(limits=c("CH6","CH5","CH4","CH3","CH2","CH1")) +
  scale_y_continuous(limits=c(0,240), expand=c(0,0), breaks=c(0,25,50,75,100,125,150,175,200,225)) +
  theme_bw() +
  coord_flip() +
  facet_grid(time~., scales="free_x") +
  labs(x="", y="Abundance (count)", title="Amphipod composition") +
  theme(legend.position = "right", plot.title = element_text(size=20), axis.text = element_text(size = 15), 
        strip.text = element_text(size=15), axis.title = element_text(size =15),
        legend.text = element_text(size=15), plot.margin=unit(c(0.25,-0.2,0.25,0.7), unit="cm"))

length(which(guts.melt$predator_id == "C.floridanus"))
length(which(guts.melt$predator_id == "C.pseudogracilis"))

guts = guts.melt[guts.melt$value > 0,] %>% mutate(order=factor(order, levels=c("Crangonyctidae", "Corophiida", "Cladocera", "Cyclopoida", "Diptera", "Mollusca", "Oligochaeta", "Tricladida"))) %>% ggplot(aes(site, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=order)) +
  scale_fill_manual(values=cols_gut, name = "") +
  scale_x_discrete(limits=c("CH6","CH5","CH4","CH3","CH2","CH1")) +
  scale_y_continuous(expand=c(0,0)) +
  theme_bw() +
  coord_flip() +
  facet_grid(time~predator_id) +
  labs(x="", y="Composition (ratio)", title="Gut contents") +
  theme(legend.position = "right", plot.title = element_text(size=20), axis.text = element_text(size=15),
        strip.text = element_text(size=15), axis.title = element_text(size=15),
        legend.text = element_text(size=15), plot.margin=unit(c(0.25,0,0.25,0.7), unit="cm"))

edna = ggplot(melt.occup.edna, aes(site, value)) +
  geom_bar(stat="identity", position="fill", aes(fill=order)) +
  scale_fill_manual(values=cols_edna, name = "") +
  scale_x_discrete(limits=c("CH6","CH5","CH4","CH3","CH2","CH1")) +
  scale_y_continuous(expand=c(0,0)) +
  theme_bw() +
  coord_flip() +
  labs(x="", y="Composition (ratio)", title="eDNA") +
  facet_grid(time~.) +
  theme(legend.position = "right", plot.title = element_text(size=20), axis.text = element_text(size = 15), 
        strip.text = element_text(size=15), axis.title = element_text(size =15),
        legend.text = element_text(size=15), plot.margin=unit(c(0.25,1,0.25,0.7), unit="cm"))

ga = plot_grid(tot, labels=c("a)"), label_size=25, label_fontface=1)
gb = plot_grid(edna, labels=c("b)"), label_size=25, label_fontface=1)
gc = plot_grid(guts, labels=c("c)"), label_size=25, label_fontface=1)
gd = plot_grid(kick, labels=c("d)"), label_size=25, label_fontface=1)

png("R_images/barplot_community_pubblication.png", width = 4000, height = 6000, units = "px", res = 300)
grid.arrange(ga, gb, gc, gd, ncol=1, nrow=4)
dev.off()
```

#12. Sanger sequence aligment

This last chunk requires msa (R-package) and LaTex Live to be installed.
To install LaTex Live on Ubuntu 16.04 from terminal:

`$ sudo apt-get install texlive-full`

Note: having multiple LaTex versions installed can give errors.

```{r sanger alignment}
# Import of sanger sequences
barcode_alignment=readDNAMultipleAlignment("c.flor_c.pse_SANGER_aligned.fasta")
msaCheckNames(barcode_alignment, ".")

print(barcode_alignment, show="alignment", showNames=T, order="align")
rownames(barcode_alignment) = gsub("_C_", "_C.", rownames(barcode_alignment))

msaPrettyPrint(barcode_alignment, output="pdf",
               showConsensus="none",
               consensusThreshold = 25,
               showNames="left",
               showLogo="top",
               showLegend=F,
               paperWidth = 18, paperHeight = 11,
               furtherCode=c("\\nomatchresidues{White}{Gray80}{upper}{}",
                             ...
                             "\\showruler{bottom}{CHASE412_C_pseudogracilis_SANGER}"),
               askForOverwrite=FALSE)
```

#13. Haplotypes network

The file 'Crangonyx_confirmation/4-infer_phylogeny/supplementary/MT-CO1@ALL@gt90_trimmed_aln.fasta' was only modified manually to remove the " <unknown description>" from the first sequence (first line of the file), since it was giving errors in the read.dna function. The "unknown description" was introduced during ML tree construction.

```{r load DNA file for haplotypes}
seq_cfcp = read.dna("Crangonyx_confirmation/4-infer_phylogeny/supplementary/MT-CO1@ALL@gt90_trimmed_aln.fasta", format="fasta")
```

```{r setting rownames}
rownames(seq_cfcp) = gsub(" <unknown description>", "", rownames(seq_cfcp))
rownames(seq_cfcp) = gsub("_Mb-","-Mb_", rownames(seq_cfcp))

for (i in seq(1:length(rownames(seq_cfcp)))){
#  print(paste(strsplit(rownames(seq_cfcp), split="_")[[i]][2], strsplit(rownames(seq_cfcp), split="_")[[i]][3]))
  rownames(seq_cfcp)[i] = paste(strsplit(rownames(seq_cfcp), split="_")[[i]][2], strsplit(rownames(seq_cfcp), split="_")[[i]][3])
#  print("ok")
}

rownames(seq_cfcp) = gsub("Ci [A-Z0-9]{1,}","Ci", rownames(seq_cfcp))
rownames(seq_cfcp) = gsub("Uni", "UK", rownames(seq_cfcp))
```

First we need to remove the outgroup, namely _Crangonyx islandicus_

```{r remove outgroup and create network}
seq_cfcp = seq_cfcp[grep("Ci", rownames(seq_cfcp), invert=T),]

hap_cfcp = haplotype(seq_cfcp)
net_cfcp = haploNet(hap_cfcp)

ind_cfcp = with(stack(setNames(attr(hap_cfcp, "index"), rownames(hap_cfcp))),table(hap=ind, individuals=rownames(seq_cfcp)[values]))

colnames(ind_cfcp)
```

We then remove the singletons, so all haplotypes that were represented by only one sequence.

```{r remove singletons}
hap_cfcp2 = haplotype(seq_cfcp)

hap_cfcp2 = subset(hap_cfcp2, minfreq=2)
net_cfcp2 = haploNet(hap_cfcp2)

ind_cfcp2 = with(stack(setNames(attr(hap_cfcp2, "index"), rownames(hap_cfcp2))),table(hap=ind, individuals=rownames(seq_cfcp)[values]))
```

```{r plot haplotypes no single}
png("R_images/haplotype_network_publication.png", width = 3000, height = 4000, units = "px", res = 300)
plot(net_cfcp2, size = attr(net_cfcp2, "freq"), show.mutation=1, scale.ratio=0.5, cex=1.2, label=F, pie=ind_cfcp2)
text(0, -32, labels="C. floridanus", cex=2.5)
text(25, 70, labels="C. pseudogracilis", cex=2.5)
legend("topleft", colnames(ind_cfcp2), col=rainbow(ncol(ind_cfcp2)), pch=19, ncol=1, cex=1.5)
dev.off()
```
