...
predmay = which(may.post.filter[-ednamay,!c(colnames(may.post.filter) %in% c("time","site","sample_id","sample_type"))] >= 0.9, arr.ind=T)
...
predoct = which(oct.post.filter[-ednaoct,!c(colnames(oct.post.filter) %in% c("time","site","sample_id","sample_type"))] >= 0.9, arr.ind=T)
...
Initial predator assignment was done using taxa with a proportion greater or equal to 90% of each sample. We now check thos samples that had a lower values.
...
miss.may = gsub("[.]", "", rownames(may.post.filter[may.post.filter$predator_id == "NA",]))
miss.may

may.post.filter$predator_id[may.post.filter$sample_id %in% "WB507"] = "Dikerogammarus_villosus"
may.post.filter$predator_id[may.post.filter$sample_id %in% c("RB304","RB303")] = "Corophium_multisetosum"
may.post.filter$predator_id[may.post.filter$sample_id %in% c(miss.may[!miss.may %in% c("WB507","RB304","RB303")])] = "Gammarus_zaddachi"
clean.may$predator_id = may.post.filter$predator_id
...
oct.post.filter$predator_id[oct.post.filter$sample_id %in% c("GW103","GW108","GW401","GW601","WB627")] = "Dikerogammarus_villosus"
oct.post.filter$predator_id[oct.post.filter$sample_id %in% c("RB301","RB302","RB303","RB304","RB305","RB606","RB613","RB616")] = "Gammarus_zaddachi"
oct.post.filter$predator_id[oct.post.filter$sample_id %in% c("RB403","RB412")] = "Crangonyx_pseudogracilis"
clean.oct$predator_id = oct.post.filter$predator_id
```
...
water.full = full_join(edna.may, edna.oct)
