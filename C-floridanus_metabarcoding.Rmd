...
# 4. Diversity analysis in kick-samples
...
# 5. Community composition of eDNA and kick-samples
...
```{r}
nrow(abund.guts[grep("C.pseudogracilis", abund.guts$predator_id),])
nrow(abund.guts[grep("C.floridanus", abund.guts$predator_id),])
...
msaPrettyPrint(seq, output="tex", shadingMode="identical", showNames="left", 
               showConsensus="none", consensusColors = "ColdHot",
               consensusThreshold = 40,
               showLogo="top", showLegend=T, logoColors="rasmol", 
               paperWidth = 17, paperHeight = 13,
               askForOverwrite=FALSE)
texi2pdf("seq.tex", clean=T)
```

```{r test align}
### ALIGNMENT OF SEQUENCES FOR THE RAXML TREE ###
## Presenting the alignment from the Sanger sequences
#sanger_seq=readDNAMultipleAlignment("C.floridanus_confirmation/Crangonyx_barcoding/c.flor-c.pse_full_SANGER.fasta")
