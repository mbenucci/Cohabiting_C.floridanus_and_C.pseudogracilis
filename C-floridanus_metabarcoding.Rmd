...
After removing unwanted taxa we retained 95.16% and 84.01% of the total reads from respectively May and October.

```{r reads retained after clean up}
(sum(may.clean)/sum(crang.may))*100
(sum(oct.clean)/sum(crang.oct))*100
```
...
#points(meta.guts, select=which(fact_time.edna[,1]=="May"),col="black",pch=20)
#points(meta.guts, select=which(fact_time.edna[,1]=="Oct"),col="blue",pch=1)
...
ordispider(meta.kick.edna, group=fact_site.edki[,1],col=c("blue","orange"),label=T)
...
```{r}
grep("Chironomus", colnames(occup.gut.edna.kick))
```
...
occup.melt[grep("Agabus|Dytiscidae|Elmidae|Haliplidae|Haliplus|Hyphydrus|Limnius|Platambus", occup.melt$variable), "order"] = "Coleoptera"
...
occup.melt[grep("Corophium|Crangonyx|Gammaridae|Gammarus", occup.melt$variable), "order"] = "Amphipoda"
occup.melt[grep("Acanthocyclops|Cyclops|Eucyclops|Eudiaptomus|Eurytemora|Mesocyclops", occup.melt$variable), "order"] = "Cyclopoida"
occup.melt[grep("Brachycera|Ceratopogonidae|Chironomidae|Chironomus|Cricotopus|Culicoides|Dolichopodidae|Euchironomus|Nematocera|Polypedilum|Procladius|Tanytarsus|Psychodidae|Stratiomys|Tipulidae", occup.melt$variable), "order"]= "Diptera"
occup.melt[grep("Caenis|Cloeon|Procleon", occup.melt$variable), "order"] = "Ephemeroptera"
...
occup.melt[grep("Erpobdella|Glossiphonia|Haemopis|Helobdella|Placobdella", occup.melt$variable), "order"] = "Hirudinea"
...
occup.melt[grep("Ceraclea|Ecnomus|Hydropsychidae|Leptoceridae|Lype|Oecetis|Oxyethira", occup.melt$variable), "order"] = "Trichoptera"
...
col.network=c("red","red","red","red","cyan3","cyan3","cyan3","yellow","tan4","tan4","darkorchid3","cyan3","tan4","cyan3")
  
#c("yellow","yellow","tan4","tan4","darkorchid3","darkorchid3","red","yellow","cyan3","cyan3","cyan3","red","tan4","tan4","tan4","red","red","red","yellow","darkcyan","darkcyan","tan4","tan4","grey48","darkgreen","darkgreen","khaki3","yellow","yellow","darkorchid3")
```
...
#sortweb(food.web, sort.order="seq", sequence=list(seq.higher=seq.high, seq.lower=seq.low))
