...
#abund.kick = as.data.frame(abund.kick %>% group_by(site, sample_id, sample_type, time, location) %>% summarise_all(list(sum)))

# setting rownames and removing extra variables
rownames(abund.kick) = paste(abund.kick$sample_id, abund.kick$time, sep="_")
raref.kick = abund.kick[,!c(colnames(abund.kick) %in% c("site","sample_id","sample_type","time","location"))]
...
inext.kick = iNEXT(t(raref.sp.kick), q=c(0,1), datatype = "abundance", se=T, endpoint=1400)
...
fact_site.kick = matrix(c(occupancy.kick$sample_id), ncol=1)
