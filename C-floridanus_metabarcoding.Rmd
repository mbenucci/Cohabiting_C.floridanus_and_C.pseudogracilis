...
edna.summary = as.data.frame(occup.edna[,!c(colnames(occup.edna) %in% c("sample_id","predator_id"))] %>% group_by(site,time, sample_type) %>% summarise_all(funs(sum)))
kick.summary = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("site","REPLICATE"))] %>% group_by(sample_id, time,sample_type) %>% summarise_all(funs(sum)))
